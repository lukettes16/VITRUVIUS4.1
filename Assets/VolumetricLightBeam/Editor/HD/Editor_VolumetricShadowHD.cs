    ...
    public class Editor_VolumetricShadowHD : Editor_CommonHD
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (FoldableHeader.Begin(this, EditorStrings.Shadow.HeaderCamera))
            {
                ...
                EditorGUI.BeginChangeCheck();
                {
                    EditorGUILayout.PropertyField(m_DepthMapDepth, EditorStrings.Shadow.DepthMapDepth);
                }
                if (EditorGUI.EndChangeCheck())
                {
                    if (Application.isPlaying)
                    {
                        Debug.LogErrorFormat(Consts.Shadow.GetErrorChangeRuntimeDepthMapDepth(m_Targets[0]));
                    }
                }
            }
            FoldableHeader.End();
